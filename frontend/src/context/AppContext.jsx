...
export function AppProvider({ children }) {
  ...
  const refreshData = useCallback(async () => {
    try {
      // 1. Fetch Courses
      const resCourses = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/courses')
      if (resCourses.ok) {
      ...
      const [resT, resS, resU] = await Promise.all([
        fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/teachers'),
        fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/students'),
        fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/users')
      ])
      ...
      // 3. Fetch Classes
      const resClasses = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/classes')
      if (resClasses.ok) {
  ...
  const login = async (email, password, latitude = null, longitude = null) => {
    try {
      ...
      const res = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/auth/login', {
        method: 'POST',
  ...
  const createTeacher = async (data) => {
    ...
    // ── Call backend API ──────────────────────────────────────────────────────
    try {
      const res = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/teachers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const updateUser = async (role, id, data) => {
    try {
      const endpoint = role === 'teacher' ? `${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/teachers/${id}` : `${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/students/${id}`;
  ...
  const createCourse = async (data) => {
    try {
      const res = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/courses', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: data.name,
  ...
  const updateCourse = async (id, data) => {
    try {
      ...
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${dbId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      });
  ...
  const enrollStudent = async (courseId, studentId) => {
    ...
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${courseId}/enroll`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const unenrollStudent = async (courseId, studentId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${courseId}/unenroll`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId }),
      })
  ...
  const requestEnrollment = async (courseId, studentId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${courseId}/request-enroll`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId }),
      })
  ...
  const approveEnrollment = async (courseId, studentId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${courseId}/approve-enroll`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId }),
      })
  ...
  const rejectEnrollment = async (courseId, studentId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${courseId}/reject-enroll`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ studentId }),
      })
  ...
  const createClass = async (data) => {
    try {
      const res = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/classes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const activateClass = async (classId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/activate`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      })
      if (res.ok) {
  ...
  const deactivateClass = async (classId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/deactivate`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      });
      if (res.ok) {
  ...
  const joinClass = async (classId, userId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const leaveClass = async (classId, userId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/leave`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId }),
      })
  ...
  const appendTranscription = useCallback(async (classId, segment) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/transcription`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveTranscription = useCallback(async (classId) => {
    try {
      ...
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/transcription/save`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const setSummary = async (classId, summary) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/summary`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const sendQuestion = async (classId, userId, text, isQuickReply = false) => {
    try {
      ...
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/questions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId,
  ...
  const answerQuestion = async (classId, questionId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/questions/${questionId}/answer`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      })
      if (res.ok) {
  ...
  const launchAttentionCheck = async (classId, timeoutSecs = 30) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/attention-check`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const respondAttentionCheck = async (classId, checkId, userId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/attention-check/${checkId}/respond`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId }),
      })
  ...
  const completeAttentionCheck = async (classId, checkId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/${classId}/attention-check/${checkId}/complete`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      })
      const json = await res.json()
  ...
  const addCourseContent = async (courseId, formData) => {
    try {
      ...
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${dbId}/contents`, {
        method: 'POST',
        body: formData
      });
      const json = await res.json();
  ...
  const deleteCourseContent = async (courseId, contentId) => {
    try {
      const dbId = courseId.toString();
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/${dbId}/contents/${contentId}`, {
        method: 'DELETE'
      });
      if (!res.ok) {
        const json = await res.json();
  ...
  const saveGrade = async (gradeData) => {
    try {
      const res = await fetch(((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/grades', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const fetchGradesByCourse = async (courseId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/grades/course/${courseId}`);
      const json = await res.json();
      if (res.ok) {
        setGrades(prev => {
  ...
  const fetchGradesByStudent = async (studentId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/grades/student/${studentId}`);
      if (res.ok) {
        showIndexInsight(res.headers.get('X-DB-Optimization'));
        const json = await res.json();
        setGrades(prev => {
  ...
  const fetchClassesByCourse = async (courseId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/course/${courseId}`);
      if (res.ok) {
        showIndexInsight(res.headers.get('X-DB-Optimization'));
        const json = await res.json();
        setClasses(prev => {
  ...
  const fetchCoursesAdvanced = async (category, estado) => {
    try {
      let url = ((import.meta.env.VITE_API_URL || 'http://localhost:3001').replace(/\/+$/, '')) + '/api/courses?';
  ...
  const fetchCoursesByStudent = async (studentId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/courses/student/${studentId}`);
      if (res.ok) {
        showIndexInsight(res.headers.get('X-DB-Optimization'));
        return await res.json();
      }
  ...
  const fetchActiveClassesByCourse = async (courseId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/course/${courseId}?isActive=true`);
      if (res.ok) {
  ...
  const fetchParticipantHistory = async (userId) => {
    try {
      const res = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3001").replace(/\/+$/, "")}/api/classes/participant/${userId}`);
      if (res.ok) {
        showIndexInsight(res.headers.get('X-DB-Optimization'));
        return await res.json();
      }
