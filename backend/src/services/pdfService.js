...
async function generarPDFRespuesta(pqrs, respuesta, adminName) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const drawRow = (label, value, lastRow = false) => {
    ...
    doc.rect(MARGIN, y, INNER_W, 0).strokeColor(BORDER).lineWidth(0.5);
    const descHeight = doc.heightOfString(pqrs.descripcion, { width: INNER_W - 20, fontSize: 10 }) + 20;
    ...
    const respHeight = doc.heightOfString(respTexto, { width: INNER_W - 20, fontSize: 10 }) + 24;
    ...
    doc.fillColor('#1F2937').fontSize(10).font('Helvetica')
       .text(respTexto, MARGIN + 10, y + 12, { width: INNER_W - 20, lineGap: 3 });
    y += respHeight + 16;
